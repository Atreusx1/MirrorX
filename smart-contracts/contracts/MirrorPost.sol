...
mapping(uint256 => mapping(uint256 => Comment)) public comments; // postId => commentId => Comment
...
function deletePost(uint256 postId) external onlySubCommunityCreator(posts[postId].subCommunityId) {
    require(posts[postId].author != address(0), "Post does not exist");
...
function deleteComment(uint256 postId, uint256 commentId) external onlySubCommunityCreator(posts[postId].subCommunityId) {
    require(comments[postId][commentId].author != address(0), "Comment does not exist");
...
function getPostsInSubCommunity(uint256 subCommunityId) external view returns (Post[] memory) {
    ...
    for (uint256 i = 1; i <= postCount; i++) {
        if (posts[i].subCommunityId == subCommunityId && !posts[i].isDeleted) {
            count++;
        }
    }
